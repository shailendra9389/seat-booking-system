...
export class TempBufferStrategy implements IBookingStrategy {
    getType(): string {
        return 'temp_buffer';
    }
    ...
    async validate(context: BookingContext): Promise<void> {
        ...
        // Temp buffer can only be booked on NON-batch day
        const batchForDay = getBatchForDay(bookingDate);
        if (batchForDay === user.batchId) {
            throw new BadRequestError(
                'Temporary buffer seats can only be booked on non-batch days'
            );
        }

        // Same timing rules as buffer
        if (!isBufferBookingTimeValid(bookingDate)) {
            throw new BadRequestError(
                'Temporary buffer seats can only be booked 1 day in advance, after 3:00 PM'
            );
        }

        // Verify the seat is designated but has been released
        const seat = await Seat.findById(seatId);
        if (!seat) {
            throw new BadRequestError('Seat not found');
        }

        // Check that this seat has a released booking for the target date
        const releasedBooking = await Booking.findOne({
            seatId,
            date: bookingDate.startOf('day').toDate(),
            status: 'released',
        });

        if (!releasedBooking) {
            throw new BadRequestError(
                'This seat is not available as a temporary buffer. It must be a released designated seat.'
            );
        }
    }
    ...
    async execute(context: BookingContext): Promise<BookingResult> {
        ...
        // Check if seat is available (not booked by someone else)
        ...
        return {
            success: true,
            message: 'Temporary buffer seat booked successfully',
            booking,
        };
    }
}
